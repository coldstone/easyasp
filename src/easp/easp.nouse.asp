...
Class EasyAsp
	...
	Private o_md5, o_rwt, o_ext, o_regex, o_fso, o_core
	Private b_cooen, b_debug, i_rule, b_cachecore
	Private Sub Class_Initialize()
		...
		Set o_core 	= Server.CreateObject(s_dicName)
...
''{/init}
	End Sub
	'设置是否将动态载入的核心类文件读入缓存
	Public Property Let CacheCore(ByVal b)
		Dim odic, arr, i, darr(0)
		b_cachecore = b
		If b Then
			If IsN(GetApp("easyasp_core_files_code")) Then
				Set odic = Server.CreateObject(s_dicName)
				arr = Split("db,List,Md5,Fso,Upload,Tpl,Aes,JSON,Cache,Http,Xml",",")
				For i = 0 To Ubound(arr)
					odic(arr(i)) = ReadEasp(arr(i),0)
				Next
				Set darr(0) = odic
				SetApp "easyasp_core_files_code", darr
				Set odic = Nothing
			End If
		Else
			If Has(GetApp("easyasp_core_files_code")) Then
				GetApp("easyasp_core_files_code")(0).RemoveAll
			End If
			RemoveApp "easyasp_core_files_code"
		End If
	End Property
	Function ReadEasp(ByVal s, ByVal n)
		Dim p : p = s_path & IIF(n=0, "core/easp.", "easp") & s & ".asp"
		If isFile(p) Then
			'Easp.WN p
			ReadEasp = Read(p)
		Else
	...
	End Function
	'检测是否已经包含文件
	Private Function CheckInclude(ByVal s)
		CheckInclude = False
		Dim f,g
		f = ReadEasp("",1) & ReadEasp(".config",1)
		g = "<!-- *?#include +?file *?= *?""core/easp." & s & ".asp"" *?-->"
		If Test(f,g) Then CheckInclude = True
	End Function
...
'		End Select
'	End Sub
	Function db() : Set db = GetCore("db") : End Function
	Function Fso() : Set Fso = GetCore("Fso") : End Function
	Function List() : Set List = GetCore("List") : End Function
	Function Upload() : Set Upload = GetCore("Upload") : End Function
	Function Tpl() : Set Tpl = GetCore("Tpl") : End Function
	Function Aes() : Set Aes = GetCore("Aes") : End Function
	Function Json() : Set Json = GetCore("Json") : End Function
	Function Cache() : Set Cache = GetCore("Cache") : End Function
	Function Http() : Set Http = GetCore("Http") : End Function
	Function Xml() : Set Xml = GetCore("Xml") : End Function
	Function Md5(Byval s) : Md5 = GetCore("Md5").To32(s) : End Function
	Function Md5_16(ByVal s) : Md5_16 = GetCore("Md5").To16(s) : End Function
	
	'动态加载核心类
	Private Function GetCore(ByVal s)
		s = LCase(s)
		'如果文件已经被直接include
		''Execute "Set GetCore = New EasyAsp_" & s : Exit Function
		'如果当前页被使用过
		If o_core.Exists(s) Then
			If LCase(TypeName(o_core(s))) = "easyasp_" & s Then Set GetCore = o_core(s) : Exit Function
		End If
		'如果缓存中有源码就从缓存中执行源码
		If Has(GetApp("easyasp_core_files_code")) Then
			Dim odic : Set odic = GetApp("easyasp_core_files_code")(0)
			If odic.Exists(s) Then
				ExecuteGlobal GetIncCode(odic(s),0)
				Execute "Set o_core(""" & s & """) = New EasyAsp_" & s
				Set GetCore = o_core(s)
				Exit Function
			End If
		End If
		'以上都没有就从源文件读取
		Dim p : p = s_path & "core/" & "easp." & s & ".asp"
		If isFile(p) Then
			On Error Resume Next
			Include p
			Execute "Set o_core(""" & s & """) = New EasyAsp_" & s
	...
	Sub WT(ByVal s) : W "<span style=""background-color:#FF3;color:blue;margin-right:10px;"">"&HtmlEncode(s)&"</span>" : End Sub
	'格式化字符串（首下标为1）
