...
'## Update Date :   2015-07-14 6:55:55
...
'	'传入RequestHeader
	...
	Public Function GetData(ByVal uri, ByVal m, ByVal async, ByVal data, ByVal u, ByVal p)
  	...
  	'开始发起请求
		a = Split("WinHttp.WinHttpRequest.5.1 MSXML2.XMLHTTP Microsoft.XMLHTTP")
	  On Error Resume Next
		For i = 0 To Ubound(a)
  		Err.Clear
  		isWin = Left(a(i),7) = "WinHttp"
  		Set o = Server.CreateObject(a(i))
  		If isWin Then
        o.SetTimeOuts ResolveTimeout, ConnectTimeout, SendTimeout, ReceiveTimeout
        o.Option(4) = 13056 '忽略错误标志
        o.Option(6) = True '自动跳转
      End If
  		'打开远程页
  		If Easp.Has(u) And Not isWin Then
  			'如果有用户名和密码
  		...
  		Else
  			...
  			o.send
  		End If
  		If isWin Then o.WaitForResponse
			...
			If Easp.IsN(CharSet) Then
			  Text = o.responseText
				...
				If Easp.Test(Headers,"charset=([\w-]+)") Then
					CharSet = Easp.RegMatch(Headers,"charset=([\w\-]+)")(0).SubMatches(0)
				'如果是Xml文档，从文档中提取编码信息
				ElseIf Easp.Test(Headers,"Content-Type:\s?text/xml") _
               And Easp.Str.Test(Text,"^<\?xml\s+[^>]+encoding\s*=\s*""([^""]+)""[^>]*\?>") Then
					CharSet = Easp.RegMatch(Text,"^<\?xml\s+[^>]+encoding\s*=\s*""([^""]+)""[^>]*\?>")(0).SubMatches(0)
				'从文件源码中提取编码
				ElseIf Easp.Test(Text,"<meta\s+[^>]*charset\s*=\s*[""']?([\w\-]+)[""']?[^>]*>") Then
					CharSet = Easp.Str.RegMatch(Text,"<meta\s+[^>]*charset\s*=\s*[""']?([\w\-]+)[""']?[^>]*>")(0).SubMatches(0)
				End If
				'Easp.WNH CharSet
	...
	Public Function Find_(ByVal s, ByVal rule)
		If Easp.Test(s,rule) Then Find_ = Easp.RegMatch(s, rule)(0).Value
	End Function
